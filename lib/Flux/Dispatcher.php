...
class Flux_Dispatcher {
	 ...
	 * @access private
	 */
	public function loginRequired($isAjax = 0, $baseURI, $message = null, $loginModule = 'account', $loginAction = 'login')
	{
		$session = Flux::$sessionData;
		if (!$message) {
		...
		if (!$session->isLoggedIn()) {
			if (Flux::config('UseCleanUrls')) {
				...
				$loginURL = sprintf('%s/?module=%s&action=%s&isAjax=%d&return_url=%s',
					$baseURI, rawurlencode($loginModule), rawurlencode($loginAction), $isAjax, rawurlencode($_SERVER['REQUEST_URI']));
			}
