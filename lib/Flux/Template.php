...
class Flux_Template {
	...
	/**
	 * SubHeader name. The subheader file would exist under the themePath's top level
	 * and the subheaderName would simply be the file's basename without the .php
	 * extension. This name is usually 'subheader'.
	 *
	 * @access protected
	 * @var string
	 */	
	protected $subHeaderName;
	...
	/**
	 * The actual path to the Subheader file.
	 *
	 * @access protected
	 * @var string
	 */
	protected $subHeaderPath;
	...
	public function __construct(Flux_Config $config)
	{
		...
		$this->subHeaderName             = $config->get('subHeaderName');
	...
	public function render(array $dataArr = array())
	{
		...
		if (file_exists($this->headerPath) && ($this->isAjax == 0 or $this->isAjax > 1)) {
	...
	public function dateField($name, $value = null, $fowardYears = null, $backwardYears = null)
	{
		...
		// Get years.
		$years = sprintf('<select name="%s_year" class="large-4 columns">', $name);
		for ($i = $fw; $i >= $bw; --$i) {
			if ($year == $i) {
		...
		// Get months.
		$months = sprintf('<select name="%s_month" class="large-4 columns">', $name);
		for ($i = 1; $i <= 12; ++$i) {
			if ($month == $i) {
		...
		// Get days.
		$days = sprintf('<select name="%s_day" class="large-4 columns">', $name);
		for ($i = 1; $i <= 31; ++$i) {
			if ($day == $i) {
	...
	{
		$class = $bool ? 'up' : 'down';
		return sprintf('<span class="%s">%s</span>', $class, $bool ? '<i class="icon-ok"></i> Online' : '<i class="icon-remove"></i> Offline');
	}
	...
	public function redirect($location = null)
	{
		...
		if($this->isAjax){
			header("Location: $location&isAjax=1");
		}else{
