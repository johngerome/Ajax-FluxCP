	...
	public function getHTML()
	{
		...
		for ($i = $start; $i < $end; ++$i) {
			...
			if ($i == $this->currentPage) {
				$pages[] = sprintf(
					'<li class="current"><a title="Page #%d">%d</a></li>',
					$i, $i
				);
			}
			else {
				$pages[] = sprintf(
					'<li><a href="#" onclick="load_page(\'%s\')" title="Page #%d" >%d</a></li>',
					$request, $i, $i
				);
			}
		}
		...
		if ($hasPrev) {
			array_unshift($pages, sprintf('<a href="#" onclick="load_page(\'%s\')" title="Previous Pane (p#%d)" >&laquo;</a> ', $this->getPageURI($start - 1), $start - 1));
		}
		
		if ($hasNext) {
			array_push($pages, sprintf(' <a href="#" onclick="load_page(\'%s\')" title="Next Pane (p#%d)" >&raquo;</a>', $this->getPageURI($end), $end));
		}
		
		$links  = sprintf('<div class="pagination-centered"><ul class="pagination">%s</ul></div>', implode(" {$this->pageSeparator} ", $pages))."\n";
		...
		if (Flux::config('ShowPageJump') && $this->numberOfPages > Flux::config('PageJumpMinimumPages')) {
			...
			$links  .= sprintf('<div class="jump-to-page large-6 large-centered columns">%s</div>', $jump);
		}
